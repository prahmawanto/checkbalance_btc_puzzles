...
# Misalkan Anda memiliki daftar alamat dompet
addresses = [
...
# Bagilah daftar menjadi beberapa bagian
chunk_size = 20
address_chunks = [addresses[i:i + chunk_size] for i in range(0, len(addresses), chunk_size)]

# Kirim permintaan untuk setiap bagian dan tampilkan saldo akhir dalam BTC
for chunk in address_chunks:
    url = "https://blockchain.info/balance?active=" + "|".join(chunk)
    response = requests.get(url)
    if response.status_code == 200:
        balances = response.json()
        for address, info in balances.items():
            final_balance_satoshi = info['final_balance']
            final_balance_btc = final_balance_satoshi / 1e8  # konversi dari satoshi ke BTC
            print(f"Address: {address}, Final Balance: {final_balance_btc} BTC")
    else:
